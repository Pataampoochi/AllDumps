...
public class CalculatePremiumService {
	...
	public boolean validatePropertyParameters(PropertyDetails propertyDetails) {
		...
		*/
		...
		if(!(builtUpArea>=400&& builtUpArea<=15000)){
		    ...
		    if(!(reconstructionCost>=1000 && reconstructionCost<=10000)) {
		        result = false;
		    }
		    else
		    {
		        ...
		        if(!((householdValuation == Constants.MIN_HOUSEHOLD_VALUATION)||(householdValuation>=100000 && 
	...
	public double calculatePremiumByPropertyAge(PropertyDetails propertyDetails) {
		...
		int sumAssured = propertyDetails.getBuiltUpArea()*propertyDetails.getReconstructionCost() 
