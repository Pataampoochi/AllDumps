...
public class IrctcService {
	...
	public TrainInfo getTicketBookingResult(TicketBooking ticketBooking) throws ApplicationException {
		//Add code here..
		//IrctcService is = new IrctcService ();
		System.out.println("gettbr");
		
		List<TrainInfo> list=new ArrayList<TrainInfo>();
		List<TrainInfo> available=buildTrains();
		List<TrainInfo> list1 =buildTrains();
		if(ticketBooking.getFromCity() == ticketBooking.getToCity())
		{
		...
		else
		{
			...
			{
				System.out.println("class :"+ticketBooking.getTravelClass());
				System.out.println("trainsclass :"+ti.getTavelClass());
				if(ti.getTavelClass() == ticketBooking.getTravelClass() && ti.getFrom() == ticketBooking.getFromCity() && ti.getTo() == ticketBooking.getToCity())
				{
					list.add(ti);
				}
			}
		}//TODO, modify return value
		return (TrainInfo) list;
	}
	...
	public String getPNRNumber(TicketBooking ticketBooking) throws ApplicationException {
		...
		{  
			return ticketBooking.getMobileNumber().substring(0,6);
		}//TODO, modify return value
	...
	private List<TrainInfo> buildTrains() {
		...
		//pune to bangalore
		TrainInfo trainInfo1=new TrainInfo("01013", "LTT CBE SPL", "Pune", "Bangalore", "AC First Class (1A)", "1:55 AM", "20 hr 30 min", "10:25 PM", 3080);
