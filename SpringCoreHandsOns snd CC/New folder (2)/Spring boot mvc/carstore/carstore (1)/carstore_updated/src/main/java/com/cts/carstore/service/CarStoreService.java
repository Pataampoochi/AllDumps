...
public class CarStoreService {
	...
	public List<Car> getCarSearchResult(CarSearch carSearch) throws ApplicationException {
		//Add code here..
		List<Car> l=buildCars();
		List<Car> r=new ArrayList<>();
		for(Car i:l) {
			if(i.getBrandName().equals(carSearch.getBrand()) && i.getFuelType().equals(carSearch.getFuelType()) && i.getPrice()<=Double.parseDouble(carSearch.getBudgetUpto().substring(0, 1))) {
				r.add(i);
			}
		}
		return r;  //TODO, modify this return value
	}
